...
const generateDocTask = {
   ...
   * @param {{paths: string, template: string, output: string, verbose?: boolean}} params Parameters
   * @returns {Promise} Promise data { file: outputFilePath }
   */
  async generateDoc(context, params) {
    console.log(params);
    // Read doc from documentation
    const docs = await readDoc(context, { paths: params.paths, verbose: params.verbose });
